...
`MicroMeta` include two core functions: `QCAT_Meta` and `QCAT_GEE_Meta`, both of them provide various arguments for users to customize the fit: we introduce some commonly used arguments here. (For more information, type `?MicroMeta`.)
...
## Asymptotic p-value
Method = "FE-VC"
one_part_VC <- QCAT_Meta(otu.tab.lst, X , 1, Tax = Tax, Method = Method, n.perm = NULL, use.cpp = T)
...
p + ggtitle("Significant lineages for FE-VC test")
```
...
p + ggtitle("Significant lineages for Het-SKAT test")
```
#### QCAT_GEE Model 

#### Rarefy the data

The excessive zero may lead to the inflation of type one error for QCAT_GEE test. Here I conduct rarefy method to eliminate the
...
p + ggtitle("Significant lineages for zero part RE-SKAt test")
```
