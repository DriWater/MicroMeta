...
methods such as RE-MV and RE-VC tests based on the summary statistics generated
...
* `Method` Meta-analysis method to be used in this test. Including fixed effect methods such as FE-MV test and FE-VC test and random effect methods like RE-MV and RE-VC test.
...
count.genus <- count.genus[-19,] # remove extremely small  observation
meta <- meta[-19,]
```
...
## Asymptotic p-value
Method = "FE-VC"
one_part_FE <- QCAT_Meta(otu.tab.lst, X , 1, Tax = Tax, Method = Method, n.perm = NULL, use.cpp = F)
FE_sig_lineage <- one_part_FE$sig.lineage
FE_sig_lineage <- str_replace_all(FE_sig_lineage," ", "_")
#FE_sig_lineage <- FE_sig_lineage[FE_sig_lineage != "2_Bacteria"]

# the index of significant lineage in the tree
FE_sig_lineage <- unlist(lapply(1:length(FE_sig_lineage),function(X){tip_num+which(tree.vis$node.label == FE_sig_lineage[X])}))

print(one_part_FE$sig.lineage) 
```

#### Visualization

```{r fig.width= 4, fig.height= 4, fig.align='center', dpi=450, eval = FALSE}
...
## Permutation p-value
Method = "RE-MV"
one_part_RE <- QCAT_Meta(otu.tab.lst, X , 1, Tax = Tax, Method = Method, n.perm = 450, use.cpp = T)
RE_sig_lineage <- one_part_RE$sig.lineage
...
```{r}
## Permutation p-value
zero_part_RE <- QCAT_GEE_Meta(otu.tab.lst, X , 1, Tax = Tax, Method = "RE-MV", n.perm = 300, use.cpp = T)
...
microbenchmark(
   QCAT_Meta(otu.tab.lst, X , 1, Tax = NULL, Method = "RE-VC", n.perm = 100, use.cpp = T), # median 42.3772
   QCAT_Meta(otu.tab.lst, X , 1, Tax = NULL, Method = "RE-VC", n.perm = 100, use.cpp = F), # median 60.6535
   times = 4
)
```
